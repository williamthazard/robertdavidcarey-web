...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--repo-path', type=str, required=True, help="Absolute path to rdavidcarey.github.io repository root")
    ...
    def handle(self, *args, **options):
        ...
        for subdir, slug in pages_to_import:
            # Try index.md first, then <slug>.md (Bob's naming convention)
            file_path = os.path.join(repo_path, subdir, 'index.md')
            if not os.path.exists(file_path):
                file_path = os.path.join(repo_path, subdir, f'{slug}.md')
            if not os.path.exists(file_path):
                self.stdout.write(self.style.WARNING(f"File not found for page '{slug}'"))
                continue
